...
fs -rm -f /data/groups/hdp_smp/hive/aging_aau_staging/*

/*
sql 
  CREATE EXTERNAL TABLE IF NOT EXISTS hdp_smp.aging_aau_staging(account_number BIGINT, event_dt STRING, hub_id STRING, aau_records BIGINT, insert_timestamp TIMESTAMP) ROW FORMAT DELIMITED FIELDS TERMINATED BY '\t' STORED as ORC;
*/
...
aau_filtered     = FILTER postal_event_all BY postal_event_code == '07';
aau_projection   = FOREACH aau_filtered GENERATE upn;
aau_distinct     = DISTINCT aau_projection;

oc_scan_join     = JOIN first_hub_join BY upn, aau_distinct BY upn;
oc_scan_groups   = GROUP oc_scan_join BY (account_number, event_dt, hub_id);

aau_data = FOREACH oc_scan_groups
{
  distinct_upns = DISTINCT oc_scan_join.$0;
  GENERATE 
      FLATTEN(group)                AS (account_number, event_dt, hub_id), 
      FLATTEN(COUNT(distinct_upns)) AS aau_records, 
      (datetime) CurrentTime()      AS insert_timestamp;
}

store aau_data into 'hdp_smp.aging_aau_staging' using org.apache.hive.hcatalog.pig.HCatStorer();